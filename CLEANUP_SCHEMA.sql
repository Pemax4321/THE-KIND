-- CLEANUP SCRIPT - Run this FIRST if you get function immutability errors
-- This removes any conflicting objects from previous runs
...
-- Drop functions that may be causing issues
DROP FUNCTION IF EXISTS update_users_timestamp() CASCADE;

-- Drop RLS policies (they don't have dependencies)
...
-- Drop indexes (they can be recreated)
...
-- Drop tables (keep user data intact - don't drop if it has data)
-- Only drop if empty or you want to start fresh
...
-- If you want a COMPLETE clean slate, uncomment the DROP TABLE lines above
-- Then run this cleanup script, and finally run SUPABASE_SCHEMA.sql

-- All cleaned up! You can now run SUPABASE_SCHEMA.sql
